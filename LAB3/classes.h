...
template <typename T>
Node<T>* Node<T>::rotateright(Node<T>* p) // правый поворот вокруг p
{
    Node* q = p->left;
    p->left = q->right;
    q->right = p;
...
template <typename T>
Node<T>* Node<T>::rotateleft(Node<T>* q) // левый поворот вокруг q
{
    Node<T>* p = q->right;
    q->right = p->left;
    p->left = q;
...
template <typename T>
Node<T>* Node<T>::balance(Node<T>* p) // балансировка узла p
{
    fixheight(p);
    if (bfactor(p) == 2)
    {
...
template <typename T>
Node<T>*  Node<T>::insert(Node<T>*&p, T data) // вставка ключа k в дерево с корнем p
{
    if (p == nullptr) {
        p = new Node<T>;
        p->data = data;
...
//распечатывание дерева в консоль обходом ЛПК
template <typename T>
...
//распечатывание дерева в консоль обходом КПЛ
template <typename T>
...
//распечатывание дерева в консоль обходом ЛКП
template <typename T>
...
//распечатывание дерева в консоль обходом ПКЛ
template <typename T>
...
//методы для деревьев
template <typename T>
void Tree<T>::insert(Tree<T> tree,T data) // вставка ключа k в дерево с корнем p
{
